...
"""
    ...
    def finalize_config(self, dps):
        ...
        def resolve_acls(valve_cl):
            ...
            egress_acl_exact_match = False
            egress_acl_meter = False
            egress_acl_matches = {}
            egress_acl_set_fields = set()
            ...
            for port in list(self.ports.values()):
                ...
                if port.acls_out:
                    ''' ds180 - changes for acls_out'''
                    test_config_condition(self.dp_acls, (
                        'dataplane ACLs cannot be used with port ACLs.'))
                    aclso = []
                    for acl in port.acls_out:
                        matches, set_fields, meter = resolve_acl(acl, None)
                        merge_matches(egress_acl_matches, matches)
                        egress_acl_set_fields = egress_acl_set_fields.union(set_fields)
                        if meter:
                            egress_acl_meter = True
                        aclso.append(self.acls[acl])
                    port.acls_out = aclso
                    egress_acl_exact_match = verify_acl_exact_match(aclso)
                    egress_acl_enabled = True
            if self.dp_acls:
                acls = []
            ...
            if egress_acl_enabled:
                egress_acl_matches.update({'in_port': False})
            ...
            override_table_config = {
                ...
                ''' ds180 - changes for acl_egress'''
                'acl_egress': ValveTableConfig(
                    'acl_egress',
                    exact_match=egress_acl_exact_match,
                    meter=egress_acl_meter,
                    output=True,
                    match_types=egress_acl_matches,
                    set_fields=tuple(egress_acl_set_fields)),
            }
